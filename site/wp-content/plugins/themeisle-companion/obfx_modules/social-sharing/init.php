	...
	public function options() {
		...
		foreach ( $this->social_share_links as $network => $data_array ) {
			$options[] = array(
				...
				'title'       => ( $network == 'instagram' ) || ( $network == 'youtube' ) || ( $network == 'facebook' )
				|| ( $network == 'twitter' ) || ( $network == 'spotify' ) ?  'Networks' : '',
				'id'          => $network,
				'name'        => $network,
				'label'       => '<i class="socicon-' . $data_array['icon'] . '"></i>  - ' . $data_array['nicename'],
				'type'        => 'toggle',
				'default'     => ( $network == 'instagram' ) ? '1' : '',
				'class'       => 'inline-setting network-toggle',
			);

			$options[] = array(
				'title'   => ( $network == 'instagram' ) || ( $network == 'youtube' ) || ( $network == 'facebook' )
				|| ( $network == 'twitter' ) || ( $network == 'spotify' )? 'Show on' : '',
